...
class DepConstTree(val constTree: Tree[String],
  ...
  require(childParentDepMap.keys.toSeq.sorted.sameElements((0 until words.size)), "The dependency map should cover all words: " + childParentDepMap +", words: " + words.mkString(" ") + ", " + PennTreeRenderer.render(constTree));
...
object DepConstTree {
  ...
  /**
   * Extracts unlabeled dependency relations from a syntax tree. Unlike normal dependency parser, each token
   * in the tree will get a head assigned. The result is a mapping from dependees to heads.
   *
   * @param constTree a parse tree
   * @param headFinder a head finder
   *
   * @return a mapping from tokens to their heads
   */
