...
class SentenceSplitter(val featureIndexer: Indexer[String], val weights: Array[Double]) {
  ...
  def tokenize(sentences: Array[String]): Array[Array[String]] = {
    ...
    sentences.map(tokenizer.tokenize(_).toArray(new Array[String](0)));        
  }
}
...
//      featStrs += ("PrevWordNextWord=" + pw + "-" + fw);
  ...
  def trainSentenceSplitter {
    ...
    val (trainExamples, testExamples) = if (SentenceSplitterTokenizerDriver.trainFromConll) {
      val reader = new ConllDocReader(SentenceSplitterTokenizerDriver.langauge, true)
      (readExamplesFromConll(reader.loadDocsFromFolderSimple(new File(SentenceSplitterTokenizerDriver.conllTrainPath), ConllDocReader.suffixFilenameFilter("auto_conll"), SentenceSplitterTokenizerDriver.conllTrainSize)),
       readExamplesFromConll(reader.loadDocsFromFolderSimple(new File(SentenceSplitterTokenizerDriver.conllTestPath), ConllDocReader.suffixFilenameFilter("auto_conll"), SentenceSplitterTokenizerDriver.conllTestSize)))
    } else {
  ...
  private def readExamplesFromConll(docs: Seq[BaseDoc]): Seq[SplitExample] = {
