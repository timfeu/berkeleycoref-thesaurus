...
object NumberGenderComputer {
  ...
  def readBergsmaLinData(fileName: String): NumberGenderComputer = readBergsmaLinData(new File(fileName))

  def readBergsmaLinData(uri: URI): NumberGenderComputer = readBergsmaLinData(new File(uri))

  def readBergsmaLinData(file: File): NumberGenderComputer = {
    Logger.logss("Reading Bergsma and Lin file from " + file.getName)
    if (file.getAbsolutePath.endsWith(".gz")) {
      readBergsmaLinData(new GZIPInputStream(new FileInputStream(file), 65536))
    } else {
      readBergsmaLinData(new FileInputStream(file))
    }
  }
   ...
   */
  def readBergsmaLinData(in: InputStream): NumberGenderComputer = {
    val index = new Indexer[String];
    val numGendMap = new mutable.HashMap[BergsmaLinKey, Array[Int]]();
